...
export default class Phone extends React.Component {
    ...
    signInWithPhone = async (phoneNumber) => {
        ...
        .then(async (resp) => {
            ...
            if (resp.data.data.message === 'User Found') {
                ToastAndroid.show("This phone number is already registered", ToastAndroid.SHORT)
            } else {
                AsyncStorage.setItem('@phone', this.state.phone_number)
                try {
        ...
        <LinearGradient colors={[ '#6FC3F7', '#3269B3' ]} style={{ width: '100%', height: '100%' }}>
            ...
            <View style={{ width: '100%', alignItems: 'center', marginTop: wp('5%') }}>
                <View style={{ width: wp('60%') }}>
                    <Text style={{ fontSize: wp('7%'), color: 'white' }}>Sign Up</Text>
                </View>
                {!this.state.confirm ?
                    <>
